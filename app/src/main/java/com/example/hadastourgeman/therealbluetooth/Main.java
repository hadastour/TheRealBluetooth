package com.example.hadastourgeman.therealbluetooth;
...
//***remember the conncction will fail and app will crash if you haven't paired first
    ...
    private void update() {
        ...
        elev++;
        if (elev==90) elev=-90;
        strngl=Integer.toHexString(rngl);
        if (strngl.length()<2)
            strngl="0"+strngl;
        else if (strngl.length()>2)
            strngl=strngl.substring(strngl.length()-2);
        strngr=Integer.toHexString(rngr);
        if (strngr.length()<2)
            strngr="0"+strngr;
        else if (strngr.length()>2)
            strngr=strngr.substring(strngr.length()-2);
        sthead=Integer.toHexString(head);
        if (sthead.length()<2)
            sthead="0"+sthead;
        else if (sthead.length()>2)
            sthead=sthead.substring(sthead.length()-2);
        stelev=Integer.toHexString(elev);
        if (stelev.length()<2)
            stelev="0"+stelev;
        else if (stelev.length()>2)
            stelev=stelev.substring(stelev.length()-2);
        string= strngl+strngr+sthead+stelev;
